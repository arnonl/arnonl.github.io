...
_Caveat_: Objects cannot start with certain charecters (such as comma, space), or start with numbers. [Snake case](https://en.wikipedia.org/wiki/Snake_case) and [Camel case](https://en.wikipedia.org/wiki/Camel_case) are a good practices for naming conventions. Let's see an example:
...
The list is of all such objects constitute the _workspace_ of R. 

<br>
Alternatively you can use the shorter version, `ls()`.
...
<br>
<span style="color:blue">**Classwork/Homework**</span>: In the console, type `date` and press enter. 
Compare the output you got to the output you get for `objects`
<br>
...
Although R can try to resolve arguments on its own, the best practice is to specify it.
...
The function <span style="background-color:#e6e6e6">`c()`</span> combines values into a vector or a list. 
<br>
...
will list the _sine_ and _square root_ of each number in the vector `x`. 
...
In certain cases, when we are only interested in a sequence of integers, we can use a special operator in R - the colon. 
...
generates a logical vector corresponding to the elements of `x` that are greater than or less than 13.
...
2. What if the length of either vector (names or values) is not the same? 
...
3. Does the subsetting "all but some" operation work with names? 
...
```{r eval=TRUE, warning=FALSE}
  paste("Learning R is", c("fun", "tough"))
```
...
<br>
<span style="color:blue">**Classwork/Homework**</span>: Check the `paste` function help page. What does `paste0()` do? What is the role of the `collapse` argument? Play with the `sep` argument to include other type of delimiters (like comma etc.)
<br>
...
```{r eval=TRUE, warning=FALSE, collapse=T}
...
<small>**Note**: There is an information loss when coercing non-integer values to integer values as revealed by the last example. Also, coercion is not always possible. For instance, coercing a charecter "hello" into numeric/integer will result in warning and `NA`.</small>
...
Look for the help page of functions of the form <code>is.<<i>datatype</i>>(y)</code> , where `datatype` is one of the types we have just learned (like character, logical, etc.) and (y) is a variable name. Give an example for how you use it.
...
1. What happens if the number of rows/columns are more than the generated numbers when using the function `matrix()`?
...
```{r}
M1 <- matrix(c(1, 2, 3, 0, 1, 5, 5, 6, 0), ncol=3, byrow = T)

det(M1)
# Get the inverse of a matrix
solve(M1)
# Solve a simple system of Ax=b
b <- c(4,0,3)
solve(M1, b)
...
[More about matrix arithmatic](https://www.statmethods.net/advstats/matrix.html)
