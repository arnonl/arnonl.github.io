...
### Wrangle data (`r format(Sys.Date(), "%b %d, %y")`)

---

#### A note on tibbles

We already saw tibbles and how to convert data frames into tibbles (using `as.tibble()` or `as_data_frame()`). 

Tibbles have a refined print method that by default shows only the first 10 rows, and all the columns that fit on screen.  

We can change this default setting of the number of rows presented by the following:
...
When we want to refer to the output within the chain of piping sequence, we have to use a special placeholder `.` when we use tibble.
...
1. What is the reason that even though the `HANES_tibble` is in tibble format the resulted object `HANES_A1C` is not ? What is the right way to get it as a tibble? [A hint: recall **Lists**]
...
Comma Seperated Values (CSV) are the most common types of files. We can read a `.csv` file through the function `read.csv()`. For a higher level of control over the way data is parsed, we may use the argument `stringsAsFactors` as FALSE.
...
```{r eval=TRUE, echo=TRUE}
  ...
  # Paste it to constitute a single URL 
  URL <- paste(URL_text_1, URL_text_2, sep="")
  HANES_original <- read.csv(text = getURL(URL), stringsAsFactors = F)
  
  # and convert into tibble
  HANES_tibble <- as.tibble(HANES_original)
```


##

When the data contains character vectors, 
parsing of these vectors can be done through the parsing function:
...
# Parse whether we have non-integer numbers
  ...
  str(parse_integer(HANES_original$DMQ_5))
  problems(parse_integer(HANES_original$DMQ_5))
...
```{r  echo=TRUE}
  ...
  parse_number("$120")
  parse_number("60%")
  # Parse the grouping as used in most Europe
...
Parsing date and time is very important in health related data sets. For example, the MIMIC3 study has tons of data with time information. 
...
If the defaults doesn't work, there are several options as follows:
...
**Note**: MIMIC3 says "dates were shifted into the future by a random offset for each individual patient [to protect patient confidentiality] in a consistent manner to preserve intervals, resulting in stays which occur sometime between the years 2100 and 2200." so don't freak out looking at the dates. 
...
```{r echo=TRUE}
  ...
  head(parse_date(MIMIC3_ADM$admittime, "%Y-%m-%d"))
```
...
To use them, we have to identify the order in which year, month, and day appear in our data, then arrange “y”, “m”, and “d” in the same order. That gives the name of the lubridate function that will parse our date. 
...
To use this, we need to load the lubridate library first.
```{r}
# Load the lubridate library
library(lubridate)
# Character vector with dates of different formats
x <- c("2009-Jan-01", "09-1-2", "2009/01/03")
ymd(x)
```


Here is how it applies to the MIMIC3 data:
...
```{r  echo=TRUE}
  ...
  # Display as tibble
  as.tibble(MIMIC3_adm_dis)
  # Display the structure of 'MIMIC3_adm_dis' to illustrate the class 'difftime'
  str(MIMIC3_adm_dis)
```
...
<span style="color:blue">**Classwork/Homework**</span>: How many "days of staying"" is the most frequent? (or what is the mode of the 'time_spent' distribution?)
...
```{r  echo=TRUE, results="hold"}
  ...
  dhours(c(12, 24))
  # List the duration from 1 to 5 days
  ddays(1:5)
...
```{r eval=TRUE}
  ...
  MIMIC3_adm_dis <- MIMIC3_adm_dis %>%
                    mutate(
                        admission_time_minus_200 = ymd(admittime) - years(200),
...
Is it 365 or 366?, since some years are leap years and periods are human interpretable values 

R will throw a warning if we try to perfom this operation.

```{r eval=TRUE, message=TRUE, warning=FALSE, echo=TRUE}
  # Find years(1) / days(1)
  years(1) / days(1)
```
...
```{r eval=TRUE, message=TRUE, warning=FALSE, echo=TRUE}
  # Find the same as above but for the next year by converting to interval
...
Notice how Intervals print. They show the beginning date and end date. And also notice how the time zone changes from EST to EDT indicating that Daylight Savings has started. 
...
* The name for a new categorical variable that represents a larger class. This is called a key, and here it is `time stamp in emergency`. Lets call this variable `EdTimeStamp`.
...
# Gather edregtime and edouttime, to form a new categorical variable "EdTimeStamp" with
# value "time"
MIMIC3_ADM_gather <- MIMIC3_ADM_tibble %>% 
  gather("edregtime", "edouttime", key = "EdTimeStamp", value = "time")

# Showing table head
as.tibble(head(MIMIC3_ADM_gather))

# Showing table tail
as.tibble(tail(MIMIC3_ADM_gather))

# An example for the duplication in the number of rows for each subject_id (here for id 88)
MIMIC3_ADM_gather %>% filter(subject_id == 88) 
```
  
Another example: (generating a random data representing daily price change for three stocks)
...
For example, in the above data set, the patient denoted by `subject_id` can have several hospital admission ids `hadm_id` depending on how many times they were admitted in the hospital.
...
# Show all cases within 'group' containing the string "newrel"  
who_gathered %>% filter(grepl('newrel', group)) %>% head() 
...
# Recheck within 'group' for cases containing the string "newrel"  
who_rel_replaced %>% filter(grepl('newrel', group)) %>% head() 
```
...
```{r  echo=TRUE}
  ...
  who_seperated_by_underscore <- who_rel_replaced %>% 
    separate(group, c("new", "type", "sexageGr"), sep = "_")
...
[look for the help on the 'sep' argument when checking the help page of `separate()` ]

```{r  echo=TRUE}
  # Then we seperate by gender and age using "sep = 1"
  who_seperated_by_gender_age <- who_seperated_by_underscore %>% 
    separate(sexageGr, c("sex", "ageGr"), sep = 1)
...
Variables that allow to connect two tables are called _keys_. There are several types of keys:
...
Note that the number of entries in `MIMIC_ADM_INE_inner_join` is lower 
than it for each of the data sets (MIMIC3_ADM or MIMIC3_INE)
...
**Note**: By default, `by = NULL`, which will use all common variables that appear in both tables as keys. This is called _natural join_. We can also use `by = c("a" = "b")`. This will match variable `a` in the first table to variable `b` in the second table.
...
[working-with-dates-and-time](https://data.library.virginia.edu/working-with-dates-and-time-in-r-using-the-lubridate-package/)  
