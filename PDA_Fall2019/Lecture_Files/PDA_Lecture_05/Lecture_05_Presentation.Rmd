...
```{r loadHANES, eval=TRUE, message=FALSE, echo=FALSE}
  # Load the package RCurl
  library(RCurl)
  # Import the HANES data set from GitHub; break the string into two for readability
  # (Please note this readability aspect very carefully)
...
`ggplot()` looks for the mapped variable in the _data_ argument.
...
We will get a warning here, because mapping an unordered variable (AGEGROUP) to an ordered aesthetic (size) is not a good idea.
...
```{r eval=FALSE, message=FALSE, warning=FALSE, echo=TRUE}
  ...
  ggplot(data = HANES) + 
  geom_point(mapping = aes(x = log(A1C), y = log(UACR), 
                           size = AGEGROUP, alpha = AGEGROUP))
```

<br>

Here we also added a mapping of AGEGROUP to the alpha aesthetic, which controls the transparency of the points.
...
```{r eval=FALSE, message=FALSE, warning=FALSE, echo=TRUE}
  ...
  ggplot(data = HANES) + 
  geom_point(mapping = aes(x = log(A1C), y = log(UACR), 
                           size = AGEGROUP, alpha=reorder(AGEGROUP, desc(AGEGROUP))))
```
...
At first glance one might conclude that there are hardly people with diabetes in the age group 20-39, while there are many more people who get diabetes as they age. 

This is an example of **directed insight** obtained through visualization. But is it true? 
...
Here is an example of how to quickly plot a two layers plot using the `qplot()` function
```{r eval=FALSE}
# Plot a two layers graph
qplot(CHOLESTEROLTOTAL, HDL, data = HANES,
  geom = c("point", "smooth"))
```
...
4. For a deeper understanding of the ggplot object stracture:  
   [A layered-grammar of graphics, page 15](http://vita.had.co.nz/papers/layered-grammar.pdf)
