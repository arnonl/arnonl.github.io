...
### R fundamentals 02: Advanced data types and graphics (Jul. DD, 2018)
...
```{r eval=TRUE}
  ...
  # And append
  df <- rbind(df, tom)
  # If you get an error here check the column names carfully, do they match?
  df
```  
...
Observe that the Glycohemoglobin percentage lies between $5-6$ for most of the men. Note that R has also chosen the number of bins, $11$ by default. You can increase (or decrease) the number of bins using the "breaks" argument. There are other cool tools like `barplot()`, `boxplot()`, `pairs()` in the _graphics_ package. 
...
4. Increase the number of bins to $20$ in the above exercise.
<br>
...
```{r eval=TRUE, warning=FALSE}
  ...
  # (Just a rough line, map each point to the next in order, many ties)
