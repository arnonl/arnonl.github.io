...
```{r setDefaultPlotPart1, include=F}
# Here we set a default values for the next code chunks, 
# the code and its output will not appear in the final report because of the option: include=FALSE
...
```{r setDefaultPlotPart2, include=F}
...
* **_cex.axis_**: Size of the axis numbers (towards 0 is smaller). Just like "col" parameter has variants such as "main", "cex" also has other variants - "axis" is one of them.

* **_lty_**: Specifies the line type - solid, dashed etc. (1 is a full line, 2 is dashed etc.)

* **_pch_**: The style of the symbol. More than 35 types of symbols.  
